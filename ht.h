...
HashTable<K,V,Prober,Hash,KEqual>::~HashTable()
{
    // Delete remaining items in the table_
...
{
    size_t count = 0;
    // Go through the table and see how many exist + aren't deleted
    for (HASH_INDEX_T i=0; i < this->table_.size(); i++) {
        if (table_[i] != nullptr) {
...
void HashTable<K,V,Prober,Hash,KEqual>::insert(const ItemType& p)
{
    ...
    if (find(key) != nullptr) {
        find(key)->second = p.second;
    }
    // Insert Key
    else {
        // First, check if table needs to be resized
        double tablesize = this->table_.size();
        // Check against the alpha value to resize
        if (this->occupied/tablesize >= this->resizeAlpha_) {
            this->resize();
        }

        // Get new resized table size
        double updatedsize = this->table_.size();
        // Throw error if there is no location available
        if (this->occupied == updatedsize) {
            throw std::logic_error("Table is full");
        }

        // Once table has been resized (if necessary), insert the item
        HashItem* newItem = new HashItem(p);
        this->table_[probe(key)] = newItem;
        this->occupied++;
    }
}
...
void HashTable<K,V,Prober,Hash,KEqual>::resize()
{
    ...
    // Re-probe all values in the old table into the new one
    for (HASH_INDEX_T i=0; i < temp.size(); i++) {
        if (temp[i] != nullptr && temp[i]->deleted == true) {
            // Updated occupied count and delete item
            this->occupied--;
            delete temp[i];
        }
        else if (temp[i] != nullptr && temp[i]->deleted == false) {
            // Re-probing
            HashItem* newitem = new HashItem(temp[i]->item);
            this->table_[probe(temp[i]->item.first)] = newitem;
        }
    }
}
